...
var time = process.uptime(),
  ...
  let k = args[0];
   if(args[0] == "list"){
    const alime = (await axios.get('https://raw.githubusercontent.com/IADSHAUISDG/sdasdas/main/list.json')).data
  ...
  var id = Math.floor(Math.random() * 848) +1
    ...
    const lengthchar = (await axios.get('https://raw.githubusercontent.com/IADSHAUISDG/sdasdas/main/data_1.json')).data
...
let l1 = await loadImage(pathAva);
    ...
    ctx.fillStyle = lengthchar[id].colorBg;
    ctx.fillText("Mirai Project", 800, 340);
    ctx.beginPath();
  ////////////////////////////////////////
...
registerFont(__dirname + `/tad/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "45px time";
    ctx.fillText("Pmh_14", 930, 540)
    ctx.fillText("Pmh_2008", 930, 610)
    ctx.fillText("Pmh_208", 930, 690)
    ctx.restore();
    ctx.save();
    ctx.beginPath();
  ...
  return api.sendMessage({
    body: `Đã Hoạt Động:${hours < 10 ? (hours > 0 ? " 0" + hours + " giờ" : "") : (hours > 0 ? " " + hours + " giờ" : "")} ${minutes < 10 ? (minutes > 0 ? " 0" + minutes + " phút" : "") : (minutes > 0 ? " " + minutes + " phút" : "")}${seconds < 10 ? (seconds > 0 ? " 0" + seconds + " giây" : "") : (seconds > 0 ? " " + seconds + " giây" : "")}\n\nSố người dùng: ( ${global.data.allUserID.length} )\nSố nhóm: ( ${global.data.allThreadID.length} )\nModules: ( ${commands.size} )\nPrefix box: ( ${prefix} )\nID Nhân Vật: ( ${id} )`,
