...
registerFont(__dirname + `/tad/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "55px time";
    ctx.fillText("Triệu Tài Tân", 930, 540)
    ctx.fillText("Triệu Tài Tân", 930, 610)
    ctx.fillText("Triệu Tài Tân", 930, 690)
    ctx.restore();
    ctx.save();
    ctx.beginPath();
  ...
  return api.sendMessage({
    body: `•======{ 𝙐𝙋𝙏𝙄𝙈𝙀 𝘽𝙊𝙏 }======•\n\nChip: ${chips}.\nTốc độ xử lý: ${speed}MHz.\n\nTổng bộ nhớ: ${byte2mb(os.totalmem())}.\nĐã sử dụng: ${byte2mb(os.freemem())} (${(os.freemem() * 100 / os.totalmem()).toFixed()}%).\n\nBot đang chạy lên ${hours} hours ${minutes} minute ${seconds} seconds.\n\n❯ Tổng số người dùng: ${global.data.allUserID.length}\n❯ Tổng số nhóm: ${global.data.allThreadID.length}\n❯ CPU đang được sử dụng: ${pidusage.cpu.toFixed(1)}%\n❯ Ram đang được sử dụng: ${byte2mb(pidusage.memory)}\n❯ Ping: ${Date.now() - timeStart}ms\n❯ ID nhân vật: ${id}\n❯ Admin Faceboook:\nhttps://www.facebook.com/NguyenTanTaiSupport`,
