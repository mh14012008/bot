...
var full = [];
                ...
                switch (args[0]) {
                    ...
                    case "tôm":
                        case "Tôm": itemm = "tom";
                                icon = '🦞';
                            break;
                                default: return api.sendMessage("[:<🐧] => Hãy Bấm : /baucua [bầu/cua/cá/nai/gà/tôm] [số tiền] biết chưa đồ ngốc",event.threadID, event.messageID);
                }      
                await get(number[0],number[1],number[2]);
            api.sendMessage({body:"[:<🐧] => Đang Đập, À Không Đang Lắc Để Cho Mấy Con Nghiện chơi!",attachment: createReadStream(__dirname + "/cache/baucua.gif")},event.threadID,async (error,info) => {
    ...
    var array = [number[0],number[1],number[2]];
        ...
        if (array.includes(itemm)) {
            ...
            return api.sendMessage({body:`[:<🐧] => Kết Quả : ${full.join("|")}\n[✤] => 𝙏𝙧𝙪̛̀ ${args[1]} Đô,Vì Có 0 ${icon}!`,attachment: listimg},event.threadID, event.messageID);
        }
            } ,event.messageID);
    };
