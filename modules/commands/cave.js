...
var tdh = Math.floor(Math.random() * 70000000) + 20000; //random coins khi làm cave ở Trần Duy Hưng
var pnl = Math.floor(Math.random() * 70000000) + 20001; //random coins khi làm cave ở Phạm Ngũ Lão
var nht = Math.floor(Math.random() * 70000000) + 20000; //random coins khi làm cave ở Nguyễn Huy Tự
//random ku dài
var tdhs = ['10cm', '20cm', '30cm', '40cm', '50cm', '1km',]; //random độ dài của ku 
var ku1 = tdhs[Math.floor(Math.random() * tdhs.length)];   

var pnls = ['10cm', '20cm', '30cm', '40cm', '50cm', '1km']; //random độ dài của ku
var ku2 = pnls[Math.floor(Math.random() * pnls.length)]; 

var nhts = ['10cm', '20cm', '30cm', '40cm', '50cm',
'1km']; //random độ dài của ku
...
var msg = "";
    switch(handleReply.type) {
        case "choosee": {
            ...
            switch(event.body) {
                case "1": msg = `⚡️𝑆𝑎𝑢 1 đ𝑒̂𝑚 𝑏𝑎̣𝑛 đ𝑢̛𝑜̛̣𝑐 𝑎ℎ 𝑐𝑢 𝑑𝑎̀𝑖   ${ku2} ở 𝐓𝐫𝐚̂̀𝐧 𝐃𝐮𝐲 𝐇𝐮̛𝐧𝐠 𝑣𝑎̀ 𝑘𝑖𝑒̂́𝑚 đ𝑢̛𝑜̛̣𝑐 ${pnl} 𝙑𝙉𝘿`; Currencies.increaseMoney(event.senderID, pnl); break;            
                case "2": msg = `⚡️𝑆𝑎𝑢 1 đ𝑒̂𝑚 𝑏𝑎̣𝑛 đ𝑢̛𝑜̛̣𝑐 𝑎ℎ 𝑐𝑢 𝑑𝑎̀𝑖   ${ku2} ở 𝐏𝐡𝐚̣𝐦 𝐍𝐠𝐮̃ 𝐋𝐚̃𝐨 𝑣𝑎̀ 𝑘𝑖𝑒̂́𝑚 đ𝑢̛𝑜̛̣𝑐 ${pnl} 𝙑𝙉𝘿`; Currencies.increaseMoney(event.senderID, pnl); break;
                case "3": msg = `⚡️𝑆𝑎𝑢 1 đ𝑒̂𝑚 𝑏𝑎̣𝑛 đ𝑢̛𝑜̛̣𝑐 𝑎ℎ 𝑐𝑢 𝑑𝑎̀𝑖   ${ku3} ở 𝐍𝐠𝐮𝐲𝐞̂̃𝐧 𝐇𝐮𝐲 𝐓𝐮̛̣ 𝑣𝑎̀ 𝑘𝑖𝑒̂́𝑚 đ𝑢̛𝑜̛̣𝑐 ${nht} 𝙑𝙉𝘿`; Currencies.increaseMoney(event.senderID, nht); break;
            };
...
module.exports.run = async ({  
    ...
    if (typeof data !== "undefined" && cooldown - (Date.now() - data.work2Time) > 0) {
        ...
        return api.sendMessage(`💎𝐁𝐚̣𝐧 đ𝐚̃ 𝐥𝐚̀𝐦 𝐜𝐚𝐯𝐞 𝐯𝐚̀ 𝐛𝐚̣𝐧 đ𝐚̃ 𝐡𝐞̂́𝐭 𝐯𝐨̂́𝐧 𝐭𝐮̛̣ 𝐜𝐨́ 𝐛𝐚̣𝐧 𝐯𝐮𝐢 𝐥𝐨̀𝐧𝐠 𝐪𝐮𝐚𝐲 𝐥𝐚̣𝐢 𝐬𝐚𝐮 𝐧𝐡𝐞́ 🎋.`, e.threadID, e.messageID);//('mày hãy quay lại sau ${housr} giờ ${minutes} phút ${seconds} giây nữa')
    }
    else {    
        var msg = {
