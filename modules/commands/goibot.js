...
module.exports.handleEvent = function({ api, event, args, Threads }) {
  ...
  var tl = ["chào bạn tôi là bot của Hiếu", "bạn kêu tôi có việc gì?", "tôi yêu bạn vler", "Yêu em >3", "Hi, chào con vợ bé:3", "Vợ gọi có việc gì không?", "Sử dụng callad để liên lạc với admin!", "Anh là bot cute nhất hành tinh", "Nói gì thế con lợn", "Anh đây~~~~", "Yêu anh Hiếu nhất", "em ấy là bae của ADMIN", "Yêu thương admin nhất", "Anh ấy là phụ trợ của admin", "Sao thế công chúa", "Đừng làm anh bùn ~~~", "Chơi đọc chữ với anh nhé a á á á", "Tuyển phi công nè", "Làm siu nhưn ko? dui lắm", "Cậu cô đơn ko?", "Set rela ko vã quá!!!", "Được của ló :)))", "Anh dthw như chủ của anh", "Đừng khen anh ngại quá hí hí" ,"Làm vợ anh ko ?", "Đừng spam anh nha :<<, anh mệt lắm ời", "bla bla", "Đừng đè anh mạnh!!!", "Đánh tutu thôi anh đau :'(", "Yêu cậu như một cực hình\nNhấp lên nhấp xuống hai mình cùng chơi", "Spam cc cút", "Yêu anh ko?", "Vợ anh đây rồi"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

  if ((event.body.toLowerCase() == "bot ngu")) {
    return api.sendMessage("👹Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: Hiếu để được hỗ trợ\nLink fb nè: https://www.facebook.com/hieu200814", threadID, () =>
  ...
  if ((event.body.toLowerCase() == "bot lon")) {
    return api.sendMessage("👹Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: TÀI để được hỗ trợ\nLink fb nè: https://www.facebook.com/hieu200814", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
...
Tài để được hỗ trợ\nLink fb nè: https://www.facebook.com/nguyenthanhmai.info1", threadID, () =>
  ...
  if ((event.body.toLowerCase() == "anh tài") || (event.body.toLowerCase() == "anh tài")) {
  ...
  if ((event.body.toLowerCase() == "a Tài") || (event.body.toLowerCase() == "a tài")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  };

  if ((event.body.toLowerCase() == "tài") || (event.body.toLowerCase() == "tài")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin tao tên Tài có gì dùng lệnh ?admin list hoặc ?info để biết thêm chi tiết", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "ông Hoàn") || (event.body.toLowerCase() == "ong hoan")) {
    return api.sendMessage("️Kêu chủ của tôi gì á :)?", threadID);
  };

  if ((event.body.toLowerCase() == "Anh Hoàn") || (event.body.toLowerCase() == "anh hoan")) {
  ...
  if ((event.body.toLowerCase() == "ko ai thương t hết") || (event.body.toLowerCase() == "ko ai thuong t het")) {
    return api.sendMessage("️Thôi ngoan nào bot thương bạn mà ❤️ ", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "bot có link fb của admin ko") || (event.body.toLowerCase() == "bot co link fb của admin ko")) {
    return api.sendMessage("Dĩ nhiên rồi có gì liên hệ anh ấy nha <3\nLink fb nè: https://www.facebook.com/NguyenTanTaiSupport", threadID);
  };
